export type SignalSource = 'Tech' | 'Research' | 'Jobs' | 'Funding' | 'Grants' | 'Community';
...
export interface Signal {
  ...
  content: string;
  timestamp: string;
  /** Where this signal actually came from, e.g. "Hacker News", "arXiv", "SEC EDGAR (Form D)". */
  origin?: string;
  /** Link back to the original item, when we have one. */
  url?: string;
}
...
export interface Opportunity {
  ...
  /** 'live' = generated by Gemini from live signals, 'cached' = static fallback example. */
  sourced?: 'live' | 'cached';
}
...
// Last-resort fallback if the live /api/signals fetch fails entirely (offline dev, all
// upstream sources down, etc.) — clearly cached, not presented as fresh.
export const FALLBACK_SIGNALS: Signal[] = [
  { id: 'fb-1', source: 'Tech', origin: 'Cached snapshot', content: 'Large internet companies are struggling to keep servers cool during unexpected heatwaves.', timestamp: '2 days ago' },
  { id: 'fb-2', source: 'Research', origin: 'Cached snapshot', content: 'Recent papers point to growing interest in ultra-low-power edge computing for remote environmental sensors.', timestamp: '4 days ago' },
  { id: 'fb-3', source: 'Jobs', origin: 'Cached snapshot', content: 'Hospitals are hiring engineers to update old systems ahead of post-quantum security requirements.', timestamp: '1 week ago' },
  { id: 'fb-4', source: 'Funding', origin: 'Cached snapshot', content: 'Investors are putting money into companies that track whether local farms have enough water reserves.', timestamp: '2 weeks ago' },
  { id: 'fb-5', source: 'Grants', origin: 'Cached snapshot', content: 'Government grant activity is picking up for software that helps small manufacturers with safety paperwork.', timestamp: '3 weeks ago' },
  { id: 'fb-6', source: 'Community', origin: 'Cached snapshot', content: 'Open source maintainers are highlighting the need for better license compliance tooling for AI-generated code.', timestamp: '3 days ago' },
];

// Deprecated alias kept for any lingering imports — prefer FALLBACK_SIGNALS.
export const RECENT_SIGNALS = FALLBACK_SIGNALS;
